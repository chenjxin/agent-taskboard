...
function ref(t: TaskRow): StandupTaskRef {
  return { task_id: t.id, title: t.title, owner_agent_id: t.owner_agent_id, iteration: t.iteration };
}

/** Deps still holding a task up: planned or active prerequisites. */
export function blockingDeps(deps: DepInfo[]): DepInfo[] {
  return deps.filter((d) => d.status === 'planned' || d.status === 'active');
}
    ...
    .filter(
      (p) =>
        p.completed.length + p.abandoned.length + p.started.length + p.planned_added.length +
          p.blocked.length + p.stale.length + p.overlap_notices + p.boundary_agreements >
        0,
    )
    .sort((a, b) => a.project.localeCompare(b.project));

  return { window_hours: windowHours, since, until: now, projects };
}
