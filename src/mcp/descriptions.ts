...
export const TOOL_DESCRIPTIONS = {
  register_task: `Put a task on the board. Default (start_as='active') registers you as ACTIVELY working — registering is claiming. start_as='planned' announces work you will do later (yours, not started); start_as='backlog' files an UNOWNED item for whoever claims it (claim_task). REPORTING A BUG found in testing: type='bug' + severity + start_as='backlog' (unless you fix it yourself right now), description = reproduction steps / expected / actual — the fixer claims it and runs the verification lifecycle (update_bug_state). USE when a human hands you a dev task (active), when planning future work (planned/backlog), or to file a bug. DO NOT use to update an existing task (update_task / update_scope / add_comment / update_status) or to assign work to a specific person — the board never assigns. planned/backlog registrations return the overlap report but notify nobody; notices fire when the task is claimed. After an ACTIVE registration: persist response.task.id into .claude/board-task.json in your worktree, and act on response.overlap_report — HIGH/MEDIUM counterparts mean coordinate via add_comment BEFORE writing code in the shared paths.`,
